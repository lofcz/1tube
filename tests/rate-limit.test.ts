...
let suffix = 0;
async function freshLimiter(): Promise<typeof import("../src/gateway/rate-limit.ts")> {
  suffix++;
  return await import(`../src/gateway/rate-limit.ts?test=${suffix}-${crypto.randomUUID()}`);
}

function buildApp(
  mw: import("hono").MiddlewareHandler,
  tagger?: (c: import("hono").Context, n: import("hono").Next) => Promise<void> | void,
) {
  const app = new Hono();
...
Deno.test("rate-limit: blocks once tokens are exhausted (anon, default rpm)", async () => {
  ...
  const limiter = createRateLimiter({ defaultRpm: 3, overrides: {}, maxBuckets: 100 });
  const app = buildApp(limiter);

  for (let i = 0; i < 3; i++) {
    const res = await app.fetch(new Request("http://localhost/functions/v1/foo"));
    assertEquals(res.status, 200, `request ${i + 1} should pass`);
  }
  const blocked = await app.fetch(new Request("http://localhost/functions/v1/foo"));
...
Deno.test("rate-limit: per-user keying uses the userId set by upstream auth", async () => {
  resetTubeEnv();
  const { createRateLimiter } = await freshLimiter();
  const limiter = createRateLimiter({ defaultRpm: 2, overrides: {}, maxBuckets: 100 });
  ...
  await app.fetch(new Request("http://localhost/functions/v1/foo"));
  const aliceBlocked = await app.fetch(new Request("http://localhost/functions/v1/foo"));
  assertEquals(aliceBlocked.status, 429);

  // Bob has his own fresh bucket.
  currentUser = "bob";
  const bobOk = await app.fetch(new Request("http://localhost/functions/v1/foo"));
...
Deno.test("rate-limit: ignores X-Forwarded-For when no trusted proxies are configured", async () => {
  resetTubeEnv();
  const { createRateLimiter } = await freshLimiter();
  const limiter = createRateLimiter({ defaultRpm: 1, overrides: {}, maxBuckets: 100 });
  const app = buildApp(limiter);
...
Deno.test("rate-limit: per-function rpm override applies independently of default", async () => {
  ...
  const blocked = await app.fetch(new Request("http://localhost/functions/v1/hot"));
  assertEquals(blocked.status, 429);

  // Other functions still pass freely.
  const cool = await app.fetch(new Request("http://localhost/functions/v1/cool"));
...
Deno.test("rate-limit: bucket eviction keeps the map within maxBuckets", async () => {
  resetTubeEnv();
  const { createRateLimiter } = await freshLimiter();
  const limiter = createRateLimiter({ defaultRpm: 1, overrides: {}, maxBuckets: 3 });
  const app = buildApp(limiter);

  // Each fn name produces a distinct bucket key (anon-keyed).
  for (const name of ["a", "b", "c", "d", "e"]) {
    const res = await app.fetch(new Request(`http://localhost/functions/v1/${name}`));
  ...
  const aAgain = await app.fetch(new Request("http://localhost/functions/v1/a"));
  assertEquals(aAgain.status, 200);
  const eBlocked = await app.fetch(new Request("http://localhost/functions/v1/e"));
