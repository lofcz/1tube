    ...
    try {
      for (const slotOffset of [0, 500]) {
        for (let i = 0; i < span; i++) {
          listeners.push(
            Deno.listen({ hostname: "127.0.0.1", port: base + slotOffset + i }),
          );
        }
      }
      return base;
...
Deno.test(
  "workerd-e2e: gateway proxies hello + echo through workerd",
  E2E_OPTS,
  async () => {
    ...
    try {
      ...
      const echoRes = await fetch(
        `http://127.0.0.1:${port}/functions/v1/echo?x=1&y=2`,
        {
          method: "POST",
      ...
      const whoRes = await fetch(
        `http://127.0.0.1:${port}/functions/v1/whoami`,
        {
      ...
      const envRes = await fetch(
        `http://127.0.0.1:${port}/functions/v1/env-probe`,
        {
          cache: "no-store",
        },
      );
      ...
      assertEquals(
        lines.length,
        N,
        `expected ${N} NDJSON lines, got ${lines.length}`,
      );
      for (let i = 0; i < N; i++) {
      ...
      assert(
        ...
        `first chunk arrived at ${firstChunkAt.toFixed(0)}ms; ` +
          `must be < ${
            latestFirstChunkAllowed.toFixed(0)
          }ms to prove streaming, not buffering`,
      );
      ...
      const zodOk = await fetch(
        `http://127.0.0.1:${port}/functions/v1/zod-deps`,
        {
          method: "POST",
      ...
      const trippedRes = await fetch(
        `http://127.0.0.1:${port}/functions/v1/boom`,
        {
          cache: "no-store",
        },
      );
      ...
      const noSuchRes = await fetch(
        `http://127.0.0.1:${port}/functions/v1/no-such-fn`,
        {
          cache: "no-store",
        },
      );
...
Deno.test(
  "workerd-e2e: shared module top-level state runs in gateway runtime",
  E2E_OPTS,
  async () => {
    if (!(await workerdAvailable())) {
      console.log("[skipped: workerd binary not on PATH]");
...
`,
    ...
    const teePump = (async () => {
      ...
      try {
        while (true) {
          ...
          if (done) break;
          Deno.stderr.writeSync(
            new TextEncoder().encode(dec.decode(value).replace(/^/gm, "   ⏵ ")),
          );
        }
      } catch { /* */ }
    })();
    ...
    try {
      ...
      const first = await fetch(
        `http://127.0.0.1:${port}/functions/v1/needs-profile`,
        {
          cache: "no-store",
        },
      );
 ...
 */
Deno.test(
  "workerd-e2e: HMR rebundles + swaps without dropping requests",
  E2E_OPTS,
  async () => {
    if (!(await workerdAvailable())) {
      console.log("[skipped: workerd binary not on PATH]");
...
`;
      ...
      while (performance.now() < pollDeadline) {
        const res = await fetch(`http://127.0.0.1:${port}/functions/v1/hello`, {
          cache: "no-store",
        });
 ...
 */
Deno.test(
  "workerd-e2e: gateway auto-recovers when workerd is killed externally",
  E2E_OPTS,
  async () => {
    if (!(await workerdAvailable())) {
      console.log("[skipped: workerd binary not on PATH]");
    ...
    try {
      ...
      assert(
        ...
        `expected last_reload_duration_ms > 0, got ${
          healthAfterBody.workerd!.last_reload_duration_ms
        }`,
      );
      assert(
        typeof healthAfterBody.workerd!.bundle_bytes.hello === "number" &&
          healthAfterBody.workerd!.bundle_bytes.hello > 0,
        `expected bundle_bytes.hello > 0, got ${
          JSON.stringify(healthAfterBody.workerd!.bundle_bytes)
        }`,
      );
      ...
      assert(
        /^onetube_workerd_bundle_bytes\{function="hello"\} \d+$/m.test(
          metricsText,
        ),
