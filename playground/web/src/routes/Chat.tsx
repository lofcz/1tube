...
export function Chat() {
  ...
  const apiKey = useMemo(() => getApiKey(selected.provider), [selected.provider, streaming]);
  ...
  const send = async () => {
    ...
    const userTurn: ChatTurn = { id: crypto.randomUUID(), role: "user", text: input };
    const assistantTurn: ChatTurn = { id: crypto.randomUUID(), role: "assistant", text: "" };
    ...
    try {
      ...
      for await (const chunk of textStream) {
        setTurns((prev) =>
          prev.map((t) => (t.id === assistantTurn.id ? { ...t, text: t.text + chunk } : t)),
        );
      }
    } catch (err) {
      ...
      setTurns((prev) =>
        prev.map((t) =>
          t.id === assistantTurn.id ? { ...t, text: t.text || `[error] ${msg}` } : t,
        ),
      );
    } finally {
  ...
  return (
    <section className="card chat">
      <div className="chat-controls">
        ...
        <button type="button" className="ghost" onClick={clear} title="Clear conversation">
        ...
        {turns.length === 0 ? (
          ...
          turns.map((t) => (
            <div key={t.id} className={`turn turn-${t.role}`}>
              ...
              <div className="turn-text">{t.text || (streaming ? "…" : "")}</div>
            </div>
          ))
        )}
      </div>
      ...
      >
        <textarea
          ...
          placeholder={apiKey ? "Type a message…  (Enter to send, Shift+Enter for newline)" : "Add an API key in Settings to start"}
          rows={3}
        ...
        {streaming ? (
          ...
          <button type="submit" className="primary" disabled={!input.trim() || !apiKey}>
            <Send size={16} /> send
          </button>
        )}
      </form>
